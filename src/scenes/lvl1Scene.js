...
export default class Lvl1Scene extends Phaser.Scene {
    ...
    preload(){
        ...
        this.load.image('lvl1bg', 'assets/images/Levels/LVL1/Lvl 1.jpg');
    ...
    create(){
        ...
        this.physics.world.setBounds((this.game.canvas.width - 1280) / 2, 0, 1280, 965, true, true, true, true);
        this.cameras.main.setBounds(100, 0, 1280, 965);
        this.cameras.main.setSize(this.game.canvas.width,this.game.canvas.height);
        this.cameras.main.setZoom(this.game.canvas.height/965) //2

        this.guernica = this.make.image({
            x: (this.game.canvas.width - 1280) / 2,
            y: 0,
            key: 'lvl1bg',
            add: true
        }).setOrigin(0);
