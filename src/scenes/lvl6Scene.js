...
export default class Lvl6Scene extends Phaser.Scene {
    ...
    preload(){
        ...
        this.load.image('lvl6bg', 'assets/images/Levels/LVL6/Lvl 6.jpg');
    ...
    create(){
        ...
        this.physics.world.setBounds((this.game.canvas.width - 1289) / 2, 0, 1289, 1030, true, true, true, true);
        this.cameras.main.setBounds(0, 0, 1289, 1030);
        this.cameras.main.setSize(this.game.canvas.width,this.game.canvas.height);
        this.cameras.main.setZoom(this.game.canvas.height/1030) //2

        this.guernica = this.make.image({
            x: (this.game.canvas.width - 1289) / 2,
            y: 0,
            key: 'lvl6bg',
            add: true
        }).setOrigin(0);
